...
# Overview 
In this vignette, we will be performing functional enrichment analysis on a differential gene expression dataset. The dataset compares the expression of transcripts in lung cancer biopses versus normal tissue. Differential expression analysis has already been performed, generating log2foldchange and P-values for each gene. The enrichment analysis will be performed against Gene Ontology, as an introduction to the most common type of enrichment, sometimes referred to as *GO Analysis*. We will review this common approach, including some basic plotting. This will serve as the foundation for more advanced enrichment analysis against a pathway database, which is called *Pathway Analysis*.

Working with pathways opens up more analysis and visualization options. We will query WikiPathways for relevant content and import pathway models into Cytoscape. In Cytoscape, we will perform data overlays, add drug interactions and generate high-quality images for publication.
...
Tables are fine, but we're in R to see plots. Conveniently, **clusterProfilier** provides a variety of plots with default settings:
```{r}
...
ewp.up <- clusterProfiler::enricher(
        ...
        universe = bkgd.genes.entrez[[2]],
        pAdjustMethod = "fdr",
        pvalueCutoff = 0.5, #p.adjust cutoff
        TERM2GENE = wpid2gene,
        TERM2NAME = wpid2name)
...
ctl.extend.cmd = paste0('cytargetlinker extend idAttribute="Ensembl" linkSetFiles="', drugbank, '"') 
