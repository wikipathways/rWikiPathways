...
Together, *BridgeDbR* provides convience to the typical *rWikPathways* user by supplying
formal names and codes defined by BridgeDb and used by WikiPathways.
...
### Getting started
Lets first check some of the most basic functions from each package. For example, 
here's how you check to see which species are currently supported by WikiPathways:
...
However, some function want the organism code, rather than the full name. Using 
BridgeDbR's *getOrganismCode* function, we can get those:
```{r}
    unlist(lapply(orgNames,BridgeDbR::getOrganismCode))
```

### Identifier System Names and Codes
Even more obscure are the various datasources providing official identifiers and
how they are named and coded. Fortunately, BridgeDb defines these clearly and simply.
And WikiPathways relies on these BridgeDb definitions. 
...
It's "L" because the resource used to be named "Locus Link". Sigh... Don't try 
to guess these codes. Use this function from BridgeDb (above) to get the correct
code. By the way, all the systems supported by BridgeDb are here:
...
Here are some specific combo functions that are useful. They let you skip worrying
about system codes altogether!
...
And if you ever find yourself with a system code, e.g., from a rWikiPathways return
result and you're not sure what it is, then you can use this function:
